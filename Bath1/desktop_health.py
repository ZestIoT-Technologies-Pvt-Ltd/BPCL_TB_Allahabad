...
import error
import subprocess
error_file="/home/zestiot/BPCL/BPCL_final/error_code.txt"
last_event="/home/zestiot/BPCL/BPCL_final/last_event.txt"
def health():
    try:
    ...
    try:
        ...
        CPU=Popen(['grep','Tdie'],stdin=CPU.stdout,stdout=PIPE)
        CPU=(CPU.communicate()[0].decode('utf-8')).split("  ")[4]
        #print (CPU)
    except Exception as e:
        print (str(e))
        CPU = "34C"
...
def apicall():
    try:
        sc = ClientSocket(device_id=str('BPCL_ALH_NX_0001'))
    except Exception as e:
...
def net_event(sc):
    try:
        ...
        with open("/home/zestiot/BPCL/BPCL_final/net_event.txt","r+") as f:
            for i in f.readlines():
                ...
                msg = sc.receive()
                print(msg)
                subprocess.call(["sed -i 1d /home/zestiot/BPCL/BPCL_final/net_event.txt"],shell=True)
    except Exception as e:
